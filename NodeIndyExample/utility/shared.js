...
async function getPoolHelender(){
    ...
    let poolHandle = await indy.openPoolLedger(config.poolName);
    return poolHandle;
}
...
async function createRequest(request){
    ...
    var updateRequest = {
        transcriptCredValues : {
            first_name: {raw: request.first_name, encoded: await  convert(16, "0123456789") },
            last_name: {raw: request.last_name, encoded: await  convert(16, "0123456789") }
        },
        id: request.first_name, 
        key: await convert(10, "abcdefghijklmnopqrstuvwxyz"),
...
module.exports = {
    ...
    verifierGetEntitiesFromLedger: verifierGetEntitiesFromLedger,
    proverGetEntitiesFromLedger: proverGetEntitiesFromLedger,
    getCredDef: getCredDef,
    getSchema: getSchema,
    sendNym: sendNym,
    convert: convert,
    sendCredDef: sendCredDef,
